...
class URLProcessor: TextProcessorExtension {
    ...
    func matchFound(fonudText :String) -> AnyObject?{
        ...
        returnDictionary["url"] = fonudText
        do{
            if let url = NSURL(string: fonudText){
                let html = try String(contentsOfURL: url, encoding: NSUTF8StringEncoding)
                let t = TextProcessor()
                t.addTextPattern(HTMLTitleProcessor())
                t.addTextPattern(HTMLImageProcessor())
                t.addTextPattern(HTMLDescriptionProcessor())
                let found = t.processText(html)
                for item in found{
                    if item.1.count>0{
                        returnDictionary[item.0] = item.1[0]
                    }
                }
            }
        }catch{
